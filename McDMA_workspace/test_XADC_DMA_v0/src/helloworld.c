...
// Define the GPIO pin for button BTN
...
static float Xadc_RawToVoltageAUX12(u16 RAWDATA) {
...
static int ActivateXADCInput1()
{
    ...
    if (ActivateXADCInput == VAUX14) {
        ...
        Xadc_RawToVoltageFunc = Xadc_RawToVoltageAUX12;
...
//    }
    ...
    if (XSysMon_SetSingleChParams(&XADCInstance, XSM_CH_AUX_MIN + 14, 0, 0, 0) != XST_SUCCESS) {
        printf("Failed to set VAUX12 as input!\r\n");
    ...
    printf("VAUX12 is activated as the input\r\n");
    ...
    while (1) {
        ...
        buttonState = XGpio_DiscreteRead(&Gpio,2);

        xil_printf("btn: %d\r\n", buttonState);

        // Check if BTN is pressed (buttonState will be non-zero when the button is pressed)
        if (buttonState) {
            ...
            for (int i = 0; i < SAMPLE_COUNT; i++) {
                float voltage = Xadc_RawToVoltageAUX12(DataBuffer[i]);  // Convert raw data to voltage
...
//	    u32 button;
//    XGpio_SetDataDirection(&Gpio, BTN_CHANNEL, 0x1);
//    while (1) {
//         // Read the button state from GPIO pin 2 (assuming BTN0_PIN is correctly defined)
