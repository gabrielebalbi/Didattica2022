...
use IEEE.NUMERIC_STD.ALL; -- Necessaria per le operazioni aritmetiche
use work.MyPkg.all;
library xil_defaultlib;

entity TDCchannel is
   generic (
      NTaps : integer := 132  -- No. of taps; multiple of 12! NTaps <= 200!
  	);
   Port ( 
      -- TDC channel input
...
begin
   ...
   process(trigger_out, reset)
       begin
           if reset = '1' then
               -- Reset del contatore a zero
               count_reg <= (others => '0');
           elsif rising_edge(trigger_out) then               
                   count_reg <= count_reg + 1;
           end if;
       end process;
   
       -- Conversione del segnale interno nell'uscita del porto
       ro_clk <= count_reg(7);
