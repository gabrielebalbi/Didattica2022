...
module IcapConfigSequencer (
    ...
    // External Control
    input go, // Go command trigger per la generazione di IPROG


	output irig_b_out,  // Segnale IRIG-B (impulso)
	output led0,		//
	output ref_pps_out, // Segnale 1 PPS (per riferimento nel testbench)
...
//						icap_in <= 32'h00000000;
	...
	irig_b_generator #(
		...
		.IRIG_BIT_PERIOD_CLKS(12_000_000)
	)
	generator_0 (
        .clk(flash_clk),         //
        .reset(rst),	    // Reset attivo alto che si trasforma in attivo basso per la logica interna
		.irig_b_out(irig_b_out),  // Segnale IRIG-B (impulso)
        .pps_led(led0),		//
        .ref_pps_out(ref_pps_out) // Segnale 1 PPS (per riferimento nel testbench)
    );
