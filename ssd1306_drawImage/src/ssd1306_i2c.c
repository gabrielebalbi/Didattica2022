...
void ssd1306_command(unsigned int c)
{
	...
	//XIic_WriteByte(i2cd, control, c, 1);
	XIic_SimpleWriteByte(i2cd, control, c);
...
void ssd1306_display(void)
{
	...
	//	XIic_WriteByte(i2cd, 0x40, buffer[i], 1);
		XIic_SimpleWriteByte(i2cd, 0x40, buffer[i]);
...
//		return;
	...
	if (n >0)
		ssd1306_fillRect(0,y,WIDTH,y+7,BLACK);
...
void ssd1306_draw_bitmap(const unsigned char *bitmap_data, int width, int height) {
    // Per Page Addressing Mode:
    // Devi impostare la pagina e la colonna di partenza per ogni blocco di dati.
    // L'SSD1306 ha 8 pagine (0-7) per un'altezza di 64 pixel.

    // Comandi per impostare la colonna di partenza (bassa e alta)
    // e la pagina di partenza. Questi comandi devono essere inviati per ogni pagina.
    // In Page Addressing Mode, una volta impostata la colonna e la pagina,
    // i dati vengono scritti sequenzialmente all'interno di quella pagina
    // fino alla fine della riga, poi si passa automaticamente alla pagina successiva
    // se non si reimposta la pagina.

    for (int page = 0; page < (height / 8); page++) { // Per ogni pagina (tipicamente 8)
        ssd1306_command(0xB0 + page); // Set Page Start Address for Page Addressing Mode (0xB0-0xB7)
        ssd1306_command(0x00);         // Set Lower Column Start Address for Page Addressing Mode (0x00-0x0F)
        ssd1306_command(0x10);         // Set Higher Column Start Address for Page Addressing Mode (0x10-0x1F)

        ssd1306_command(0x40);



        for (int col = 0; col < width; col++) { // Per ogni colonna (tipicamente 128)
            // L'indice nel tuo array bitmap_data dipenderà da come l'hai organizzato.
            // Se è page-major, colonna-major: bitmap_data[page * width + col]
        	ssd1306_command(bitmap_data[page * width + col]);
        	//usleep(100);
        }
    }
}
...
void ssd1306_drawBitmap_G(int16_t x, int16_t y, uint8_t *bitmap, int16_t w,int16_t h, uint16_t color) {
